...
const RecommendedCarousel = ({
  ...
  // Calcula ancho de scroll según breakpoint
  const getCardWidth = () => {
    ...
    >
      <div className="w-full relative">
        <h2
          ...
          className="text-3xl font-bold py-4 pl-24 text-white text-left
                     max-md:pl-3 max-md:text-lg md:pl-19 md:text-xl lg:pl-22
                     xl:text-2xl xl:pl-23"
        >
          {title}
        </h2>
        ...
        <div className="relative bg-black">
          ...
          <button
            onClick={scrollLeft}
            className="absolute md:left-4 lg:left-6 top-1/2 -translate-y-1/2
                       bg-gray-700 hover:bg-gray-600 text-white md:p-2 lg:p-3
                       rounded-full shadow-lg z-10 focus:outline-none
          ...
          <div
            ref={containerRef}
            id="carousel-rec"
            className="w-screen flex overflow-x-auto scroll-smooth hide-scroll-bar
                       pb-4 max-md:pt-1 max-md:px-3 md:pt-6 md:px-19 lg:px-22
                       xl:px-23 relative carousel-container"
            style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
          >
            {shuffledItems.map((item, i) => (
              <div
                key={i}
                className="recommended-card flex-none bg-gray-800 rounded-lg shadow-md
                           overflow-hidden transform transition-transform duration-300
                           hover:scale-105 max-md:h-[16rem] max-md:w-[10rem]
                           max-md:mr-3 md:h-[18rem] md:w-[12rem] md:mr-4
              ...
              >
                <div className="img-container w-full h-full relative">
                  ...
                  <div className="recommended-title-overlay transition-all duration-300
                                  max-md:px-1 max-md:py-2 md:py-[0.75rem] md:px-2"
                  >
                    <h3 className="line-clamp-1 max-md:text-sm xl:text-base font-bold">
                      {item.title}
                    </h3>
                  ...
                  <div className="recommended-desc flex flex-col">
                    ...
                    <span className="text-sm max-lg:line-clamp-6 max-xl:line-clamp-10">
                      {item.desc}
                    </span>

                    {/* Botón "Comenzar" con navegación */}
                    <Link
                      to={`/series/${item.id}`}
                      className="btn-comenzar flex items-center
          ...
          <button
            onClick={scrollRight}
            className="absolute md:right-4 lg:right-6 top-1/2 -translate-y-1/2
                       bg-gray-700 hover:bg-gray-600 text-white md:p-2 lg:p-3
                       rounded-full shadow-lg z-10 focus:outline-none
                       focus:ring-2 focus:ring-blue-500 focus:ring-opacity-75
                       transition-all duration-300 max-md:hidden"
